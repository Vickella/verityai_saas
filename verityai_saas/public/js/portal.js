(() => {
  ...
  async function conversations() {
    ...
    content.innerHTML = `<form id="conversation-filter" class="va-card va-form"><div class="va-fields">${field("Search session or visitor","search",conversationFilters.search)}<div class="va-field"><label>Platform</label><select name="platform"><option value="">All</option>${["Web","WhatsApp","Desk"].map(value=>`<option ${conversationFilters.platform===value?"selected":""}>${value}</option>`).join("")}</select></div><div class="va-field"><label>Status</label><select name="status"><option value="">All</option>${["Open","Human Handoff","Closed"].map(value=>`<option ${conversationFilters.status===value?"selected":""}>${value}</option>`).join("")}</select></div></div><div class="va-actions"><button class="va-button">Apply</button><a class="va-button secondary" href="/api/method/verityai_saas.api.conversations.export_csv?${exportQuery}">Export CSV</a></div></form>${table([["Conversation",r=>`<strong>${esc(r.name)}</strong><br><span class="muted">${esc(r.user_identifier||"")}</span>`],["Platform",r=>esc(r.platform)],["Status",r=>pill(r.status)],["Handoff",r=>r.handoff?`${pill(r.handoff.status)}<br><span class="muted">${esc(r.handoff.assigned_to||"Unassigned")}</span>`:"—"],["Updated",r=>esc(r.modified)],["Actions",r=>`<div class="va-actions"><button class="va-button secondary" data-handoff-open="${esc(r.name)}">Open</button><button class="va-button secondary" data-handoff-assign="${esc(r.name)}" data-assigned="${esc(r.handoff?.assigned_to||"")}">Assign</button><button class="va-button" data-handoff-resolve="${esc(r.name)}">Resolve</button></div>`]],rows)}<div class="va-actions"><button class="va-button secondary" id="conversations-prev" ${conversationFilters.start<=0?"disabled":""}>Previous</button><button class="va-button secondary" id="conversations-next" ${data.has_more?"":"disabled"}>Next</button></div>`;
  ...
  async function usage() {
    ...
    content.innerHTML = `<form id="analytics-filter" class="va-card va-form"><h2>Analytics range</h2><div class="va-fields">${field("From","from_date",analytics.from_date,"date")}${field("To","to_date",analytics.to_date,"date")}</div><div class="va-actions"><button class="va-button">Apply range</button><a class="va-button secondary" href="/api/method/verityai_saas.api.analytics.export_workspace?${exportQuery}">Download tenant export</a></div></form><div class="va-grid"><div class="va-card va-metric"><span>Total tokens</span><strong>${number(summary.total_tokens)}</strong></div><div class="va-card va-metric"><span>Estimated cost</span><strong>${number(summary.estimated_cost)}</strong></div><div class="va-card va-metric"><span>Leads</span><strong>${number(analytics.funnel?.total)}</strong></div><div class="va-card va-metric"><span>Win rate</span><strong>${number(analytics.funnel?.win_rate)}%</strong></div></div><div class="va-grid two">${barChart(analytics.usage,"tokens","Daily tokens")}${barChart(analytics.usage,"cost","Daily AI cost")}${barChart(analytics.leads,"leads","Daily leads")}${barChart(analytics.conversations,"conversations","Daily conversations")}</div><div class="va-grid two"><div class="va-card"><h2>Conversations by channel</h2>${Object.entries(analytics.channels||{}).map(([key,value])=>`<p>${esc(key)} <strong>${number(value)}</strong></p>`).join("")||'<p class="muted">No conversations.</p>'}</div><div class="va-card"><h2>Lead funnel</h2>${Object.entries(analytics.funnel?.counts||{}).map(([key,value])=>`<p>${esc(key)} <strong>${number(value)}</strong></p>`).join("")}</div></div>`;
  ...
  async function integrations() {
    ...
    const f=d.features||{}, provider=d.provider||{}, erp=d.erpnext||{}, smtp=d.smtp||{};
  ...
  async function account() {
    ...
    const createPanel = d.can_add_workspace ? `<form id="additional-workspace" class="va-card va-form"><h2>Add workspace</h2><p class="muted">This creates another isolated assistant under ${esc(d.account_name)} and applies your current account plan limits.</p><div class="va-fields">${field("Workspace name","workspace_name")}${field("Business name","business_name")}</div><button class="va-button">Create workspace</button></form>` : `<div class="va-card"><h2>Add workspace</h2><p class="va-notice error">Your current plan workspace limit has been reached. Upgrade the plan before adding another workspace.</p></div>`;
    content.innerHTML = `<div class="va-grid two"><form id="account-form" class="va-card va-form"><h2>Account profile</h2><p><strong>${esc(d.account_name)}</strong></p><p class="muted">${esc(capacity)} · Plan ${esc(d.plan||"Not configured")}</p><div class="va-fields">${field("Billing email","billing_email",d.billing_email,"email")}${field("Phone","phone",d.phone)}${field("Country","country",d.country)}${field("Currency","currency",d.currency)}<div class="va-field"><label>Customer type</label><select name="customer_type">${["SME","Agency","Enterprise"].map(value=>`<option ${d.customer_type===value?"selected":""}>${value}</option>`).join("")}</select></div></div><button class="va-button">Save profile</button></form>${createPanel}</div>`;
  ...
  const renderers={dashboard,health,onboarding,assistant,widget,knowledge,leads,conversations,quotes,usage,billing,integrations,email,whatsapp,team,account};
