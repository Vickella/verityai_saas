(() => {
  ...
  function paymentGatewayManagement(eventTable) {
    ...
    const testForm=configured&&gateway.environment==="Test"&&data.can_configure_platform?`<div class="va-settings-block"><h3>Paynow integration test</h3><p>This gateway-only test can never activate a plan or allocate credits. <strong>EcoCash simulated success</strong> uses Paynow's official 0771111111 test number and normally becomes Paid after about five seconds. Hosted fake success requires you to sign in with the Paynow merchant account, select <strong>TESTING: Faked Success</strong>, and confirm.</p><form id="admin-paynow-test-form" class="va-form"><div class="va-fields"><div class="va-field"><label>Test workspace</label><select name="workspace" required>${workspaceOptions}</select></div><div class="va-field"><label>Paynow merchant login email</label><input name="merchant_email" type="email" required autocomplete="email" placeholder="Email used to create this integration"></div><div class="va-field"><label>Test path</label><select name="test_method"><option value="ecocash">EcoCash simulated success</option><option value="onemoney">OneMoney simulated success</option><option value="hosted">Hosted fake success</option></select></div><div class="va-field"><label>Paynow simulator number</label><input value="0771111111" readonly aria-describedby="paynow-test-number-help"><small id="paynow-test-number-help">Fixed by Paynow for successful Test-mode simulations; no real money moves.</small></div></div><div class="va-form-actions"><button class="va-button">Start test transaction</button>${paynowTestPayment?`<button type="button" class="va-button secondary" id="poll-paynow-test">Check Paynow result</button>`:""}</div></form></div>`:"";
  ...
  function bindActions() {
    ...
    paynowTestForm?.addEventListener("submit",async event=>{
      ...
      try{const result=await call("verityai_saas.api.admin.start_paynow_test",values);paynowTestPayment=result.payment;if(result.checkout_url){window.location.assign(result.checkout_url);return;}const url=new URL(location.href);url.searchParams.set("paynow_test",result.payment);history.replaceState({},"",url);alert("Paynow simulation started. Wait about 5 seconds, then select Check Paynow result.");render();}
    ...
    document.querySelector("#poll-paynow-test")?.addEventListener("click",async event=>{
      ...
      try{const result=await call("verityai_saas.api.admin.poll_paynow_test",{payment:paynowTestPayment});alert(result.status==="Completed"?"Paynow test completed successfully. In Paynow, open Integration Keys and select Request to be Set Live.":`Paynow test status: ${result.gateway_status||result.status}. Wait a few seconds and check again if it is still pending.`);if(result.status==="Completed"){paynowTestPayment="";history.replaceState({},"",location.pathname);}await load(selectedWorkspace);}
