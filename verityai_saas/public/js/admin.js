(() => {
  ...
  function paymentGatewayManagement(eventTable) {
    ...
    const testForm=configured&&gateway.environment==="Test"&&data.can_configure_platform?`<div class="va-settings-block"><h3>Paynow integration test</h3><p>Creates a USD 1.00 gateway-only transaction. Sign in to Paynow with the merchant account that owns this integration, choose <strong>TESTING: Faked Success</strong>, then select <strong>Make Payment</strong>. This test never changes a customer plan or allocates AI credits.</p><form id="admin-paynow-test-form" class="va-form"><div class="va-fields"><div class="va-field"><label>Test workspace</label><select name="workspace" required>${workspaceOptions}</select></div><div class="va-field"><label>Paynow merchant login email</label><input name="merchant_email" type="email" required autocomplete="email" placeholder="Email used to create this integration"></div></div><div class="va-form-actions"><button class="va-button">Create Paynow test</button>${paynowTestPayment?`<button type="button" class="va-button secondary" id="poll-paynow-test">Check Paynow result</button>`:""}</div></form></div>`:"";
    ...
    return `<div class="va-payment-admin-layout">${gatewayCard}<aside class="va-payment-readiness"><p class="eyebrow">Gateway status</p><h3>${live?"Production mode enabled":gateway.environment==="Production"?"Credentials required":"Test mode"}</h3><p>${live?"Customer checkouts use Paynow's signed hosted-payment flow. Paynow must also have approved this integration ID for live payments.":gateway.environment==="Production"?"Save valid Paynow credentials to enable checkout.":"Use the Paynow integration test on this page. Customer checkout stays disabled and test payments cannot activate subscriptions."}</p></aside></div>${section("Billing ledger","Recent payments, top-ups, refunds and provider updates.",eventTable,`<a class="va-button secondary" href="/api/method/verityai_saas.api.billing.reconciliation_export">Export reconciliation</a>`)}`;
  }
