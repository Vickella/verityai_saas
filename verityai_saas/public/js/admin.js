(() => {
  ...
  function render() {
    ...
    const views={overview,workspaces:`<div class="va-admin-search"><div><p class="eyebrow">Tenant operations</p><strong>${number(filtered.length)} of ${number(workspaces.length)} workspaces</strong></div><div class="va-field"><label>Search workspaces</label><input id="admin-workspace-search" value="${esc(workspaceSearch)}" placeholder="Business, workspace, status or plan"></div></div>${section("Customer workspaces","Review health, plan, usage and operational signals.",workspaceTable)}`,plans:planManagement(),reports:scheduleManagement(),billing:`<div class="va-status-strip"><div><span class="va-status-dot ${data.paynow_configured?"good":""}"></span><div><strong>Paynow gateway</strong><p>Manual actions remain operator-only and every mutation is written to the billing ledger.</p></div></div><div class="va-actions">${pill(data.paynow_configured?"Connected":"Not configured")}<a class="va-button secondary" href="/api/method/verityai_saas.api.billing.reconciliation_export">Export reconciliation</a></div></div>${section("Billing ledger","Recent payments, top-ups, refunds and provider updates.",eventTable)}`};
