(() => {
  ...
  const section=(title,description,body,actions="")=>`<section class="va-section"><header class="va-section-heading"><div><h2>${esc(title)}</h2><p>${esc(description)}</p></div>${actions?`<div class="va-actions">${actions}</div>`:""}</header>${body}</section>`;
  ...
  function paymentGatewayManagement(eventTable) {
    ...
    const gatewayForm=data.can_configure_platform?`<form id="admin-paynow-form" class="va-payment-gateway-form va-form"><div class="va-settings-block"><div class="va-fields"><div class="va-field"><label>Paynow integration ID</label><input name="integration_id" required maxlength="140" value="${esc(gateway.integration_id||"")}" autocomplete="off" placeholder="e.g. 12345"></div><div class="va-field"><label>Integration key ${configured?'<span class="va-field-state">Configured</span>':""}</label><input name="integration_key" type="password" ${configured?"":"required"} autocomplete="new-password" placeholder="${configured?"Leave blank to keep the current key":"Paste the key from Paynow"}"></div></div><div class="va-secret-hint"><strong>Secret handling</strong><span>The integration key is encrypted at rest, write-only, and never returned to this page.</span></div></div><div class="va-form-actions"><button class="va-button">${configured?"Save gateway changes":"Connect Paynow"}</button></div></form>`:`<div class="va-callout warning"><strong>Platform administrator required</strong><span>Only the Frappe Administrator or an authorised SaaS administrator can change payment credentials.</span></div>`;
    const gatewayCard=`<section class="va-payment-gateway-card"><div class="va-payment-gateway-brand"><div class="va-payment-mark" aria-hidden="true">P</div><div><p class="eyebrow">Payment gateway</p><h2>Paynow</h2><p>Accept secure hosted checkout without handling card numbers inside VerityAI.</p></div><div class="va-payment-connection"><span class="va-status-dot ${configured?"good":""}"></span>${pill(configured?"Connected":"Not configured")}</div></div><div class="va-payment-methods"><div><span class="va-method-icon">CARD</span><strong>Debit & credit cards</strong><small>Visa and Mastercard through Paynow hosted checkout</small></div><div><span class="va-method-icon">MOB</span><strong>Mobile & local methods</strong><small>Available methods are selected securely on Paynow</small></div></div>${gatewayForm}<footer><span>Credential source</span><strong>${esc(gateway.source||"Not configured")}</strong></footer></section>`;
    return `<div class="va-payment-admin-layout">${gatewayCard}<aside class="va-payment-readiness"><p class="eyebrow">Go-live checklist</p><h3>Ready to take payments</h3><ul><li class="${configured?"done":""}">Paynow credentials saved</li><li>Merchant verified for Visa / Mastercard</li><li>USD subscription prices published</li><li>Production return URL is publicly reachable</li></ul><p>Card availability is controlled by your Paynow merchant account. VerityAI redirects customers to Paynow so card details never touch this server.</p></aside></div>${section("Billing ledger","Recent payments, top-ups, refunds and provider updates.",eventTable,`<a class="va-button secondary" href="/api/method/verityai_saas.api.billing.reconciliation_export">Export reconciliation</a>`)}`;
  }
  ...
  function bindActions() {
    ...
    paynowForm?.addEventListener("submit",async event=>{
      event.preventDefault();const button=paynowForm.querySelector("button");button.disabled=true;button.textContent="Saving securely...";
      const values=Object.fromEntries(new FormData(paynowForm).entries());
      try{const status=await call("verityai_saas.api.admin.configure_paynow",{values});data.paynow=status;data.paynow_configured=Boolean(status.configured);alert("Paynow gateway configured. Card checkout is now available to customers.");render();}
