(() => {
  ...
  function growthManagement() {
    ...
    const doctorWorkspaceOptions=(data.workspaces||[]).filter(row=>row.status==="Active").map(row=>`<option value="${esc(row.name)}" ${doctor.crm_workspace===row.name?"selected":""}>${esc(row.business_name||row.workspace_name)} · ${esc(row.name)}</option>`).join("");
