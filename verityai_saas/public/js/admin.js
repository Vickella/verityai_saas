(() => {
  ...
  const growthFlagLabels={
    growth_foundation_enabled:["Growth foundation","Channel governance, attribution and safety records"],
    website_doctor_internal_enabled:["Internal Website Doctor","Allow platform administrators to run controlled audit pilots"],
    public_audits_enabled:["Public Website Doctor","Allow public audit requests after its release gate passes"],
    website_builder_enabled:["Website builder","Allow customer projects after builder acceptance passes"],
    partner_portal_enabled:["Partner portal","Allow approved partners to access their scoped portal"],
    white_label_enabled:["White label","Resolve approved partner branding and hostnames"],
    outbound_enabled:["Outbound delivery","Permit approved delivery adapters; keep disabled during foundation work"],
  };

  function channelEditor(channel=null) {
    const item=channel||{}; const create=!channel;
    const options=(values,current)=>values.map(value=>`<option ${current===value?"selected":""}>${esc(value)}</option>`).join("");
    return `<form class="va-admin-editor" data-growth-channel="${esc(item.name||"")}"><details><summary><div><span class="va-plan-label">${create?"New channel":esc(item.channel_code)}</span><strong>${create?"Register a growth channel":esc(item.channel_name)}</strong><small>${create?"Add a governed acquisition or delivery surface":`${esc(item.delivery_mode)} · ${esc(item.risk_class)} risk`}</small></div><div class="va-actions">${create?"":pill(item.status)}<span class="va-chevron">⌄</span></div></summary><div class="va-admin-editor-body"><div class="va-settings-block"><div class="va-fields"><div class="va-field"><label>Channel name</label><input name="channel_name" required maxlength="140" value="${esc(item.channel_name||"")}"></div><div class="va-field"><label>Channel code</label><input name="channel_code" ${create?"required":"disabled"} maxlength="40" value="${esc(item.channel_code||"")}" placeholder="e.g. PARTNER_WEBINAR"></div><div class="va-field"><label>Channel type</label><select name="channel_type">${options(["Owned","Product","Messaging","Partner","Community","Marketplace","Integration"],item.channel_type||"Owned")}</select></div><div class="va-field"><label>Delivery mode</label><select name="delivery_mode">${options(["Public","Automated","Human Operated","Human Approved","Embedded","Partner Managed"],item.delivery_mode||"Human Operated")}</select></div><div class="va-field"><label>Risk class</label><select name="risk_class">${options(["Low","Moderate","High"],item.risk_class||"Low")}</select></div><div class="va-field"><label>Status</label><select name="status">${options(["Active","Paused","Archived"],item.status||"Active")}</select></div><div class="va-field"><label>Owner user</label><input name="owner_user" value="${esc(item.owner_user||"")}" placeholder="Optional user ID or email"></div><div class="va-field"><label>Monthly delivery limit</label><input name="monthly_limit" type="number" min="0" step="1" value="${esc(item.monthly_limit||0)}"><small>0 means no channel-level limit. Feature-specific limits still apply.</small></div><div class="va-field full"><label>Allowed actions</label><textarea name="allowed_actions" maxlength="1000" placeholder="What this channel is permitted to do">${esc(item.allowed_actions||"")}</textarea></div><div class="va-field full"><label>Operating notes</label><textarea name="notes" maxlength="2000">${esc(item.notes||"")}</textarea></div></div></div><div class="va-form-actions"><button class="va-button">${create?"Register channel":"Save governance"}</button></div></div></details></form>`;
  }

  function campaignEditor(campaign=null) {
    const item=campaign||{}; const create=!campaign;
    const channelOptions=(data.growth?.channels||[]).map(row=>`<option value="${esc(row.name)}" ${item.channel===row.name?"selected":""}>${esc(row.channel_name)}</option>`).join("");
    const options=(values,current)=>values.map(value=>`<option ${current===value?"selected":""}>${esc(value)}</option>`).join("");
    return `<form class="va-admin-editor" data-growth-campaign="${esc(item.name||"")}"><details><summary><div><span class="va-plan-label">${create?"New campaign":esc(item.campaign_code)}</span><strong>${create?"Create a measurable campaign":esc(item.campaign_name)}</strong><small>${create?"Use one attribution contract across every channel":`${esc(item.objective)} · ${esc(item.channel)}`}</small></div><div class="va-actions">${create?"":pill(item.status)}<span class="va-chevron">⌄</span></div></summary><div class="va-admin-editor-body"><div class="va-settings-block"><div class="va-fields"><div class="va-field"><label>Campaign name</label><input name="campaign_name" required maxlength="140" value="${esc(item.campaign_name||"")}"></div><div class="va-field"><label>Campaign code</label><input name="campaign_code" ${create?"required":"disabled"} maxlength="40" value="${esc(item.campaign_code||"")}" placeholder="e.g. WHATSAPP_PROOF_SEP26"></div><div class="va-field"><label>Channel</label><select name="channel" required><option value="">Select channel</option>${channelOptions}</select></div><div class="va-field"><label>Objective</label><select name="objective">${options(["Awareness","Acquisition","Activation","Referral","Revenue","Retention"],item.objective||"Acquisition")}</select></div><div class="va-field"><label>Status</label><select name="status">${options(["Draft","Active","Paused","Completed","Archived"],item.status||"Draft")}</select></div><div class="va-field"><label>Audience</label><input name="audience" maxlength="1000" value="${esc(item.audience||"")}" placeholder="e.g. Zimbabwean service SMEs"></div><div class="va-field"><label>Source</label><input name="source" value="${esc(item.source||"")}" placeholder="e.g. linkedin"></div><div class="va-field"><label>Medium</label><input name="medium" value="${esc(item.medium||"")}" placeholder="e.g. organic_social"></div><div class="va-field"><label>Content</label><input name="content" value="${esc(item.content||"")}" placeholder="Optional asset variant"></div><div class="va-field"><label>Term</label><input name="term" value="${esc(item.term||"")}" placeholder="Optional topic or keyword"></div><div class="va-field"><label>Starts on</label><input name="starts_on" type="date" value="${esc(item.starts_on||"")}"></div><div class="va-field"><label>Ends on</label><input name="ends_on" type="date" value="${esc(item.ends_on||"")}"></div><div class="va-field"><label>Budget (${esc(item.currency||"USD")})</label><input name="budget" type="number" min="0" step="0.01" value="${esc(item.budget||0)}"></div><div class="va-field"><label>Delivery limit</label><input name="delivery_limit" type="number" min="0" step="1" value="${esc(item.delivery_limit||0)}"></div><div class="va-field full"><label>Operating notes</label><textarea name="notes" maxlength="2000">${esc(item.notes||"")}</textarea></div></div><input type="hidden" name="currency" value="${esc(item.currency||"USD")}"></div><div class="va-form-actions"><button class="va-button">${create?"Create campaign":"Save campaign"}</button></div></div></details></form>`;
  }

  function growthManagement() {
    const growth=data.growth||{}; const metrics=growth.metrics||{}; const flags=growth.feature_flags||{};
	const doctor=growth.website_doctor_configuration||{};
    const funnelTable=table([["Channel",row=>`<strong>${esc(row.channel_name)}</strong><br><span class="muted">${esc(row.channel_code)}</span>`],["Inbound",row=>number(row.inbound_messages)],["Conversations",row=>number(row.conversations)],["Leads",row=>`<strong>${number(row.leads)}</strong><br><span class="muted">${number(row.lead_conversion_rate)}% conversion</span>`],["Qualified",row=>number(row.qualified_leads)],["Opportunities",row=>number(row.opportunities)],["Won",row=>`<strong>${number(row.won)}</strong><br><span class="muted">${number(row.win_rate)}% win rate</span>`],["Won value",row=>`USD ${Number(row.won_value||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`]],growth.channel_funnel||[],{title:"No product channels available",description:"Seed the Widget and WhatsApp channel definitions to begin attribution."});
    const websiteProjectTable=table([["Project",row=>`<strong>${esc(row.project_name)}</strong><br><span class="muted">Reserved handle: ${esc(row.project_slug)}</span>`],["Workspace",row=>esc(row.workspace)],["Definition",row=>row.current_version?`<strong>Version ${number(row.version_count)}</strong><br><span class="muted">Validated current version</span>`:`<span class="muted">Definition required</span>`],["Domain",row=>pill(row.domain_status||"Unconfigured")],["Status",row=>pill(row.status)],["Updated",row=>esc(row.modified)]],growth.website_projects||[],{title:"No website projects yet",description:"Projects will appear after the Website Builder release control is enabled and a customer creates one."});
    const websiteAuditTable=table([["Website",row=>`<strong>${esc(row.target_host)}</strong><br><span class="muted">${esc(row.name)}</span>`],["Request",row=>`${pill(row.request_kind)}<br><span class="muted">${esc(row.workspace||"Public")}</span>`],["Score",row=>row.status==="Completed"?`<strong>${number(row.overall_score)}/100</strong>`:`<span class="muted">Not available</span>`],["Measured work",row=>`${number(row.pages_checked)} page<br><span class="muted">Fetch ${number(row.fetch_duration_ms)} ms · Provider ${number(row.provider_duration_ms)} ms</span>`],["Provider cost",row=>`USD ${Number(row.provider_cost_usd||0).toFixed(6)}`],["Status",row=>`${pill(row.status)}${row.error_reference?`<br><span class="muted">Ref ${esc(row.error_reference)}</span>`:""}`],["Completed",row=>esc(row.completed_at||"—")]],growth.website_audits||[],{title:"No Website Doctor audits yet",description:"Audits will appear after an operator pilot or the public release control is enabled."});
    const websiteAuditPilot=data.can_configure_platform?`<form id="website-audit-pilot-form" class="va-form va-inline-action"><div class="va-field"><label>Public website URL</label><input type="url" name="url" required maxlength="2048" placeholder="https://example.com"></div><button class="va-button">Run internal audit</button><span class="muted">Queues one bounded, evidence-only audit. Public access remains controlled separately.</span></form>`:"";
    const templateTable=table([["Template",row=>`<strong>${esc(row.name)}</strong><br><span class="muted">${esc(row.key)} · ${esc(row.category)}</span>`],["Source",row=>pill(row.source)],["Description",row=>esc(row.description)],["Status",row=>pill(row.active===false?"Inactive":"Active")],["Action",row=>row.source==="Uploaded"&&data.can_configure_platform?`<button type="button" class="va-button secondary" data-template-active="${esc(row.key)}" data-active="${row.active===false?0:1}">${row.active===false?"Activate":"Deactivate"}</button>`:"—"]],growth.website_templates||[],{title:"No website templates",description:"Built-in templates appear after the website catalogue patch is installed."});
    const templateUpload=data.can_configure_platform?`<form id="website-template-upload-form" class="va-form va-inline-action" enctype="multipart/form-data"><div class="va-field"><label>Website template ZIP</label><input type="file" name="template" accept="application/zip,.zip" required><small>Maximum 5 MB. The archive must contain a root template.json and optional README only. Code, HTML and executable files are rejected.</small></div><button class="va-button">Validate & upload</button></form>`:"";
    const doctorWorkspaceOptions=(data.workspaces||[]).filter(row=>["Trial","Active"].includes(row.status)).map(row=>`<option value="${esc(row.name)}" ${doctor.crm_workspace===row.name?"selected":""}>${esc(row.business_name||row.workspace_name)} · ${esc(row.name)}</option>`).join("");
    const doctorConfiguration=data.can_configure_platform?`<form id="website-doctor-configuration-form" class="va-form"><div class="va-fields"><div class="va-field full"><label>Website Doctor CRM workspace</label><select name="crm_workspace"><option value="">Do not capture public leads</option>${doctorWorkspaceOptions}</select><small>Explicitly consenting Website Doctor contacts enter this workspace's existing CRM and deduplication boundary.</small></div></div><div class="va-form-actions">${pill(doctor.crm_ready?"CRM ready":"CRM mapping required")}<button class="va-button">Save CRM mapping</button></div></form>`:"";
    const flagForm=data.can_configure_platform?`${doctorConfiguration}<form id="growth-flags-form" class="va-growth-flags va-form">${Object.entries(growthFlagLabels).map(([key,value])=>`<label class="va-growth-flag"><span><strong>${esc(value[0])}</strong><small>${esc(value[1])}</small></span><input type="checkbox" name="${key}" ${flags[key]?"checked":""}></label>`).join("")}<div class="va-form-actions"><span class="muted">New customer-facing capabilities remain off until their release gates pass.</span><button class="va-button">Save release controls</button></div></form>`:`<div class="va-callout warning"><strong>Platform administrator required</strong><span>Operators can inspect growth activity but cannot change release controls.</span></div>`;
    const campaignTable=table([["Campaign",row=>`<strong>${esc(row.campaign_name)}</strong><br><span class="muted">${esc(row.campaign_code)}</span>`],["Channel",row=>esc(row.channel)],["Objective",row=>esc(row.objective)],["Attribution",row=>`<span class="muted">${esc(row.source||"—")} / ${esc(row.medium||"—")}</span>`],["Window",row=>`${esc(row.starts_on||"Open")} → ${esc(row.ends_on||"Open")}`],["Status",row=>pill(row.status)]],growth.campaigns||[],{title:"No campaigns yet",description:"Create the first measured campaign for Widget, WhatsApp or CRM."});
    const eventTable=table([["Event",row=>`<strong>${esc(row.event_type)}</strong><br><span class="muted">${esc(row.correlation_id||row.name)}</span>`],["Channel",row=>esc(row.channel||"—")],["Campaign",row=>esc(row.campaign||"—")],["Workspace",row=>esc(row.workspace||"—")],["Source",row=>`${esc(row.source||"—")} / ${esc(row.medium||"—")}`],["Occurred",row=>esc(row.occurred_at)]],growth.recent_events||[],{title:"No growth events yet",description:"Events will appear when governed product and campaign journeys begin."});
    const suppressionTable=table([["Protected identity",row=>`<code>${esc((row.identity_hash||"").slice(0,12))}…</code>`],["Channel",row=>esc(row.channel||"All channels")],["Reason",row=>esc(row.reason)],["Window",row=>`${esc(row.starts_on||"—")} → ${esc(row.expires_on||"No expiry")}`],["Status",row=>pill(row.status)],["Action",row=>row.status==="Active"&&data.can_configure_platform?`<button type="button" class="va-button secondary" data-lift-suppression="${esc(row.name)}">Lift</button>`:"—"]],growth.suppressions||[],{title:"No suppressed identities",description:"Opt-outs and prohibited recipients will be visible here without exposing their raw identifiers."});
    const privacyForms=data.can_configure_platform?`<div class="va-grid two"><form id="growth-consent-form" class="va-card va-form va-growth-action"><div><p class="eyebrow">Consent ledger</p><h3>Record permission</h3><p>Raw identifiers are converted to keyed hashes before storage.</p></div><div class="va-field"><label>Subject identifier</label><input name="identifier" required autocomplete="off" placeholder="Email, phone or opaque visitor ID"></div><div class="va-field"><label>Subject type</label><select name="subject_type">${["Visitor","User","Lead","Customer","Partner"].map(value=>`<option>${value}</option>`).join("")}</select></div><div class="va-field"><label>Purpose</label><input name="purpose" required placeholder="e.g. Product updates"></div><div class="va-field"><label>Channel</label><select name="channel"><option value="">Any applicable channel</option>${(growth.channels||[]).map(row=>`<option value="${esc(row.name)}">${esc(row.channel_name)}</option>`).join("")}</select></div><div class="va-field"><label>Source/evidence</label><textarea name="evidence" required placeholder="Where and how permission was obtained"></textarea></div><button class="va-button">Record consent</button></form><form id="growth-suppression-form" class="va-card va-form va-growth-action"><div><p class="eyebrow">Delivery safety</p><h3>Suppress an identity</h3><p>Active suppression is checked before future marketing delivery.</p></div><div class="va-field"><label>Subject identifier</label><input name="identifier" required autocomplete="off" placeholder="Email or phone"></div><div class="va-field"><label>Channel</label><select name="channel"><option value="">All channels</option>${(growth.channels||[]).map(row=>`<option value="${esc(row.name)}">${esc(row.channel_name)}</option>`).join("")}</select></div><div class="va-field"><label>Reason</label><input name="reason" required maxlength="500" placeholder="Opt-out, complaint, policy or legal restriction"></div><div class="va-field"><label>Expires on</label><input name="expires_on" type="datetime-local"></div><button class="va-button danger">Add suppression</button></form></div>`:"";
    return `<div class="va-growth-hero"><div><p class="eyebrow">Governed distribution</p><h2>One transparent growth control plane</h2><p>Product, partner and content loops share attribution, consent, release controls and human approval.</p></div><div class="va-growth-loop"><span>Product</span><span>Partner</span><span>Content</span></div></div><div class="va-grid va-growth-metrics"><div class="va-card va-metric"><span>Active channels</span><strong>${number(metrics.active_channels)}</strong><small>Governed delivery surfaces</small></div><div class="va-card va-metric"><span>Active campaigns</span><strong>${number(metrics.active_campaigns)}</strong><small>Currently operating</small></div><div class="va-card va-metric"><span>Recorded events</span><strong>${number(metrics.events)}</strong><small>Immutable funnel evidence</small></div><div class="va-card va-metric"><span>Suppressions</span><strong>${number(metrics.active_suppressions)}</strong><small>Protected identities</small></div></div>${section("Product conversion funnel","Privacy-safe attribution from Widget or WhatsApp conversation through CRM outcome.",funnelTable)}${section("Website template catalogue","Upload safe, reusable design packages. Customer projects copy validated definitions and never execute archive contents.",`${templateUpload}${templateTable}`)}${section("Website project foundation","Workspace ownership, immutable structured versions and deployment readiness without exposing unfinished publishing controls.",websiteProjectTable)}${section("Website Doctor operations","Bounded audit work, measured cost and safe error references without exposing submitted paths or evidence publicly.",`${websiteAuditPilot}${websiteAuditTable}`)}${section("Release controls","Capabilities are enabled independently after security and acceptance gates pass.",flagForm)}${section("Campaigns","Plan measurable activity across current and future Verity channels.",`${campaignTable}<div class="va-admin-editor-list">${campaignEditor()}${(growth.campaigns||[]).map(campaignEditor).join("")}</div>`)}${section("Channel governance","Every distribution surface declares its delivery mode, risk and permitted actions.",`<div class="va-admin-editor-list">${channelEditor()}${(growth.channels||[]).map(channelEditor).join("")}</div>`)}${section("Consent and suppression","Manage communication permission without exposing identifiers in operational tables.",`${privacyForms}${suppressionTable}`)}${section("Recent growth events","Canonical evidence shared by product, partner and content journeys.",eventTable)}`;
  }
  ...
  function paymentGatewayManagement(eventTable) {
    ...
    const gatewayForm=data.can_configure_platform?`<form id="admin-paynow-form" class="va-payment-gateway-form va-form"><div class="va-settings-block"><div class="va-callout ${gateway.environment==="Production"?"warning":"info"}"><strong>VerityCore mode: ${esc(gateway.environment||"Test")}</strong><span>This switch controls VerityCore only. Paynow separately changes integration ${esc(gateway.integration_id||"—")} from Test to Live after approving your successful test transaction.</span></div><div class="va-fields"><div class="va-field"><label>VerityCore checkout mode</label><select name="environment"><option value="Test" ${gateway.environment!=="Production"?"selected":""}>Test — operator simulations only</option><option value="Production" ${gateway.environment==="Production"?"selected":""}>Production — customer checkout</option></select></div><div class="va-field"><label>Paynow integration ID</label><input name="integration_id" required maxlength="140" value="${esc(gateway.integration_id||"")}" autocomplete="off" placeholder="e.g. 12345"></div><div class="va-field"><label>Integration key ${configured?'<span class="va-field-state">Configured</span>':""}</label><input name="integration_key" type="password" ${configured?"":"required"} autocomplete="new-password" placeholder="${configured?"Leave blank to keep the current key":"Paste the key from Paynow"}"></div></div></div><div class="va-form-actions"><button class="va-button">${configured?"Save Paynow settings":"Connect Paynow"}</button></div></form>`:`<div class="va-callout warning"><strong>Platform administrator required</strong></div>`;
    ...
    const testForm=configured&&gateway.environment==="Test"&&data.can_configure_platform?`<div class="va-settings-block"><h3>Paynow integration test</h3><p>This gateway-only test can never activate a plan or allocate credits. Choose any official Paynow simulator scenario for EcoCash or OneMoney. Hosted fake success instead requires the merchant-account login and manual confirmation.</p><form id="admin-paynow-test-form" class="va-form"><div class="va-fields"><div class="va-field"><label>Test workspace</label><select name="workspace" required>${workspaceOptions}</select></div><div class="va-field"><label>Paynow merchant login email</label><input name="merchant_email" type="email" required autocomplete="email" placeholder="Email used to create this integration"></div><div class="va-field"><label>Test path</label><select name="test_method"><option value="ecocash">EcoCash simulation</option><option value="onemoney">OneMoney simulation</option><option value="hosted">Hosted fake success</option></select></div><div class="va-field"><label>Paynow test scenario</label><select name="phone"><option value="0771111111">Success — 0771111111 (about 5 seconds)</option><option value="0772222222">Delayed success — 0772222222 (about 30 seconds)</option><option value="0773333333">User cancelled — 0773333333 (about 30 seconds)</option><option value="0774444444">Insufficient balance — 0774444444 (immediate)</option></select><small>These are Paynow simulator numbers. No real wallet is charged.</small></div></div><div class="va-form-actions"><button class="va-button">Start test transaction</button>${paynowTestPayment?`<button type="button" class="va-button secondary" id="poll-paynow-test">Check Paynow result</button>`:""}</div></form></div>`:"";
    ...
    return `<div class="va-payment-admin-layout">${gatewayCard}<aside class="va-payment-readiness"><p class="eyebrow">VerityCore gateway mode</p><h3>${live?"Local Production mode":gateway.environment==="Production"?"Credentials required":"Local Test mode"}</h3><p>${live?`Customer checkout is enabled locally. This does not prove Paynow has made integration ${esc(gateway.integration_id||"—")} Live; Paynow controls that status separately.`:gateway.environment==="Production"?"Save valid Paynow credentials to enable checkout.":"Operator simulations are enabled. Customer checkout is disabled and test payments cannot activate subscriptions or credits."}</p></aside></div>${section("Billing ledger","Recent payments, top-ups, refunds and provider updates.",eventTable,`<a class="va-button secondary" href="/api/method/verityai_saas.api.billing.reconciliation_export">Export reconciliation</a>`)}`;
  }
  ...
  function creditManagement() {
    ...
    const pricing=stock.provider_pricing||{};
    ...
    const purchaseForm=`<form id="credit-purchase-form" class="va-form va-credit-entry-form"><div class="va-credit-purchase-grid"><div class="va-fields"><div class="va-field"><label>Receipt type</label><select name="entry_type"><option>Purchase</option><option>Opening Balance</option></select></div><div class="va-field"><label>Currency</label><input name="currency" value="USD" readonly></div><div class="va-field"><label>Amount paid</label><input name="monetary_value" type="number" min="0.01" step="0.01" value="10.00" required></div><div class="va-field"><label>Provider model</label><input value="${esc(pricing.model||"gpt-4.1-mini")}" readonly></div><div class="va-field full"><label>Provider reference</label><input name="reference" maxlength="140" placeholder="Invoice or purchase reference"></div></div><aside class="va-credit-calculator" aria-live="polite"><span>Estimated credits to receive</span><strong id="credit-purchase-total">0</strong><div><span>Estimated credits per USD</span><b>${number(stock.purchase_rate)}</b></div><div><span>Cost per million</span><b id="credit-cost-million">USD ${Number(pricing.blended_usd_per_million||0).toFixed(2)}</b></div><div><span>Projected stock balance</span><b id="credit-projected-balance">${number(stock.balance_credits)}</b></div><div><span>Projected inventory value</span><b id="credit-projected-value">${money(stock.balance_value)}</b></div></aside></div><div class="va-form-actions"><button class="va-button">Add credit stock</button></div></form>`;
    ...
    return `${metrics}<div class="va-grid two va-credit-command-grid">${section("Add provider credits","Enter the provider amount paid. The configured model determines the estimated credits automatically.",purchaseForm)}${section("Plan unit economics","Monthly allocations use the current weighted acquisition cost.",planEconomics)}</div>${adjustmentForm}${section("Credit stock ledger","Every purchase and completed sale updates credit quantity, monetary value, cost and profit.",ledger)}${section("ERPNext accounting","Map credit sales and cost of sales to a remote ERPNext company.",accountingForm)}`;
  }

  function guideManagement() {
    const guide=data.whatsapp_setup_guide||{};
    const size=guide.file_size?`${(Number(guide.file_size)/1024/1024).toFixed(2)} MB`:"";
    const current=guide.available
      ? `<div class="va-callout info"><strong>Guide available to customers</strong><span>${esc(guide.file_name)}${size?` · ${esc(size)}`:""}${guide.modified?` · uploaded ${esc(guide.modified)}`:""}</span><a class="va-button secondary" href="${esc(guide.url)}" target="_blank" rel="noopener">Open current PDF</a></div>`
      : `<div class="va-callout warning"><strong>No guide uploaded</strong><span>The download button is hidden from customers until you upload a PDF.</span></div>`;
    const form=data.can_configure_platform
      ? `<form id="admin-guide-upload-form" class="va-payment-gateway-form va-form" enctype="multipart/form-data"><div class="va-settings-block"><div class="va-field full"><label for="admin-guide-file">WhatsApp AI setup guide</label><input id="admin-guide-file" name="guide" type="file" accept="application/pdf,.pdf" required><small>PDF only, maximum 20 MB. Uploading a new guide securely replaces the previous customer download.</small></div></div><div class="va-form-actions"><button class="va-button">${guide.available?"Replace setup guide":"Upload setup guide"}</button></div></form>`
      : `<div class="va-callout warning"><strong>Platform administrator required</strong></div>`;
    return section("Customer setup guide","Upload and control the WhatsApp AI PDF offered in every customer workspace.",`${current}${form}`);
  }
  ...
  function bindActions() {
    ...
    const growthFlagsForm=document.querySelector("#growth-flags-form");
    growthFlagsForm?.addEventListener("submit",async event=>{
      event.preventDefault();const button=growthFlagsForm.querySelector("button");button.disabled=true;button.textContent="Saving...";
      const values={};Object.keys(growthFlagLabels).forEach(key=>values[key]=growthFlagsForm[key]?.checked?1:0);
      try{data.growth.feature_flags=await call("verityai_saas.api.growth.configure_feature_flags",{values});alert("Growth release controls saved.");render();}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent="Save release controls";}
    });
    document.querySelectorAll("[data-growth-channel]").forEach(form=>form.addEventListener("submit",async event=>{
      event.preventDefault();const button=form.querySelector("button");button.disabled=true;button.textContent="Saving...";
      const values=Object.fromEntries(new FormData(form).entries());
      try{await call("verityai_saas.api.growth.save_channel",{channel:form.dataset.growthChannel,values});alert(form.dataset.growthChannel?"Channel governance updated.":"Growth channel registered.");await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent=form.dataset.growthChannel?"Save governance":"Register channel";}
    }));
    document.querySelectorAll("[data-growth-campaign]").forEach(form=>form.addEventListener("submit",async event=>{
      event.preventDefault();const button=form.querySelector("button");button.disabled=true;button.textContent="Saving...";
      const values=Object.fromEntries(new FormData(form).entries());
      try{await call("verityai_saas.api.growth.save_campaign",{campaign:form.dataset.growthCampaign,values});alert(form.dataset.growthCampaign?"Campaign updated.":"Campaign created in Draft status.");await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent=form.dataset.growthCampaign?"Save campaign":"Create campaign";}
    }));
    const growthConsentForm=document.querySelector("#growth-consent-form");
    growthConsentForm?.addEventListener("submit",async event=>{
      event.preventDefault();const button=growthConsentForm.querySelector("button");button.disabled=true;button.textContent="Recording...";
      const values=Object.fromEntries(new FormData(growthConsentForm).entries());
      try{await call("verityai_saas.api.growth.record_consent",{values});growthConsentForm.reset();alert("Consent evidence recorded securely.");await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent="Record consent";}
    });
    const growthSuppressionForm=document.querySelector("#growth-suppression-form");
    growthSuppressionForm?.addEventListener("submit",async event=>{
      event.preventDefault();if(!window.confirm("Suppress this identity from the selected delivery scope?"))return;
      const button=growthSuppressionForm.querySelector("button");button.disabled=true;button.textContent="Protecting...";
      const values=Object.fromEntries(new FormData(growthSuppressionForm).entries());
      try{await call("verityai_saas.api.growth.suppress_identity",{values});growthSuppressionForm.reset();alert("Identity added to the suppression register.");await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent="Add suppression";}
    });
	const doctorConfigurationForm=document.querySelector("#website-doctor-configuration-form");
	doctorConfigurationForm?.addEventListener("submit",async event=>{
	  event.preventDefault();const button=doctorConfigurationForm.querySelector("button");button.disabled=true;button.textContent="Saving...";
	  try{data.growth.website_doctor_configuration=await call("verityai_saas.api.growth.configure_website_doctor",{values:{crm_workspace:doctorConfigurationForm.crm_workspace.value}});alert("Website Doctor CRM mapping saved.");render();}
	  catch(error){alert(error.message,true);button.disabled=false;button.textContent="Save CRM mapping";}
	});
    const websiteAuditPilotForm=document.querySelector("#website-audit-pilot-form");
    websiteAuditPilotForm?.addEventListener("submit",async event=>{
      event.preventDefault();const button=websiteAuditPilotForm.querySelector("button");button.disabled=true;button.textContent="Queueing...";
      try{const result=await call("verityai_saas.api.audits.start_operator",{url:websiteAuditPilotForm.url.value});websiteAuditPilotForm.reset();alert(`Audit ${result.audit} queued for ${result.target_host}.`);await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent="Run internal audit";}
    });
    const websiteTemplateUploadForm=document.querySelector("#website-template-upload-form");
    websiteTemplateUploadForm?.addEventListener("submit",async event=>{
      event.preventDefault();const button=websiteTemplateUploadForm.querySelector("button");const file=websiteTemplateUploadForm.template.files[0];
      if(!file)return;if(file.size>5*1024*1024){alert("Template ZIP must be 5 MB or smaller.",true);return;}
      button.disabled=true;button.textContent="Validating...";const body=new FormData();body.append("template",file);
      try{const response=await fetch("/api/method/verityai_saas.api.admin.upload_website_template",{method:"POST",headers:{"X-Frappe-CSRF-Token":window.csrf_token||""},body});const payload=await response.json();const result=payload.message||payload;if(!response.ok||!result.success)throw new Error(result.error||`Upload failed (HTTP ${response.status}).`);alert(`${result.data.name} is ready in the website template catalogue.`);await load(selectedWorkspace);}
      catch(error){alert(error.message||"The template could not be uploaded.",true);button.disabled=false;button.textContent="Validate & upload";}
    });
    document.querySelectorAll("[data-template-active]").forEach(button=>button.addEventListener("click",async()=>{
      const active=button.dataset.active==="1";button.disabled=true;
      try{data.growth.website_templates=await call("verityai_saas.api.admin.set_website_template_active",{template_key:button.dataset.templateActive,active:active?0:1});alert(`Template ${active?"deactivated":"activated"}. Existing customer versions are unchanged.`);render();}
      catch(error){alert(error.message,true);button.disabled=false;}
    }));
    document.querySelectorAll("[data-lift-suppression]").forEach(button=>button.addEventListener("click",async()=>{
      if(!window.confirm("Lift this suppression? Future authorised campaigns may contact this identity."))return;
      button.disabled=true;button.textContent="Lifting...";
      try{await call("verityai_saas.api.growth.lift_suppression",{record:button.dataset.liftSuppression});alert("Suppression lifted with an audit timestamp.");await load(selectedWorkspace);}
      catch(error){alert(error.message,true);button.disabled=false;button.textContent="Lift";}
    }));
    const guideUploadForm=document.querySelector("#admin-guide-upload-form");
    guideUploadForm?.addEventListener("submit",async event=>{
      event.preventDefault();
      const button=guideUploadForm.querySelector("button");
      const file=guideUploadForm.guide.files[0];
      if(!file)return;
      if(file.size>20*1024*1024){alert("The setup guide must be 20 MB or smaller.",true);return;}
      button.disabled=true;button.textContent="Uploading...";
      const body=new FormData();body.append("guide",file);
      try{
        const response=await fetch("/api/method/verityai_saas.api.admin.upload_whatsapp_setup_guide",{method:"POST",headers:{"X-Frappe-CSRF-Token":window.csrf_token||""},body});
        const payload=await response.json();const result=payload.message||payload;
        if(!response.ok||!result.success)throw new Error(result.error||`Upload failed (HTTP ${response.status}).`);
        data.whatsapp_setup_guide=result.data;alert("WhatsApp setup guide uploaded. Customers can download it now.");render();
      }catch(error){alert(error.message||"The guide could not be uploaded.",true);button.disabled=false;button.textContent=data.whatsapp_setup_guide?.available?"Replace setup guide":"Upload setup guide";}
    });
    ...
    const refreshCreditPurchase=()=>{
      ...
      const rate=Number(data.credit_stock?.purchase_rate||0);
    ...
    paynowTestForm?.addEventListener("submit",async event=>{
      ...
      try{const result=await call("verityai_saas.api.admin.start_paynow_test",values);if(result.terminal){paynowTestPayment="";alert(`Paynow returned the expected ${result.expected_result} result: ${result.gateway_error||result.status}.`);await load(selectedWorkspace);return;}paynowTestPayment=result.payment;if(result.checkout_url){window.location.assign(result.checkout_url);return;}const url=new URL(location.href);url.searchParams.set("paynow_test",result.payment);history.replaceState({},"",url);const wait=values.phone==="0771111111"?"5":values.phone==="0772222222"||values.phone==="0773333333"?"30":"a few";alert(`Paynow simulation started. Wait about ${wait} seconds, then select Check Paynow result.`);render();}
    ...
    document.querySelector("#poll-paynow-test")?.addEventListener("click",async event=>{
      ...
      try{const result=await call("verityai_saas.api.admin.poll_paynow_test",{payment:paynowTestPayment});const final=["Completed","Cancelled","Failed"].includes(result.status);alert(result.status==="Completed"?"Paynow test completed successfully. In Paynow, open Integration Keys and select Request to be Set Live.":final?`Paynow returned the expected final test status: ${result.gateway_status||result.status}. Run the Success scenario before requesting Live activation.`:`Paynow test status: ${result.gateway_status||result.status}. Wait a few seconds and check again.`);if(final){paynowTestPayment="";history.replaceState({},"",location.pathname);}await load(selectedWorkspace);}
