...
from verity_ai.api.chat import assert_public_rate_limit, header_background, primary_colours, public_fallback_response, validate_public_message
...
from verity_ai.engine.openai_handler import UsageLimitExceeded, execute_tool_call, get_api_key, get_or_create_session, get_tool_definitions, process_chat
...
class TestEnterpriseHardening(FrappeTestCase):
	...
	def test_provider_key_lookup_falls_back_to_existing_openai_key(self):
		config = SimpleNamespace(get_password=lambda field, raise_exception=False: "legacy-openai-key" if field == "openai_api_key" else None)
		self.assertEqual(get_api_key(config), "legacy-openai-key")

	def test_widget_settings_fetch_remains_a_simple_cross_origin_request(self):
		widget_source = (Path(__file__).resolve().parents[1] / "public" / "js" / "widget.js").read_text(encoding="utf-8")
		self.assertIn("cache: 'no-store'", widget_source)
		self.assertIn("headers: { 'Accept': 'application/json' }", widget_source)
		self.assertNotIn("'Cache-Control': 'no-cache'", widget_source)
		self.assertIn("this.setUnavailable('This chat is not available from this website at the moment.')", widget_source)

	def test_public_fallback_does_not_expose_internal_server_messages(self):
		frappe.local.message_log = ["private provider error"]
		response = public_fallback_response("public-session")
		self.assertEqual(frappe.local.message_log, [])
		self.assertNotIn("private provider error", frappe.as_json(response))
